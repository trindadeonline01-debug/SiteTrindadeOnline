...
type Props = { id: string; initialListing: Listing }

export default function AnuncioPageClient({ id, initialListing }: Props) {
  ...
  // A sessão do site fica no localStorage do navegador, não em cookie — só
  // dá pra saber quem é o dono/admin aqui no cliente. O anúncio em si já
  // veio pronto do servidor.
  useEffect(()=>{
  ...
  async function saveEdit(){
    if(!editForm.title.trim())return; setActionLoading(true)
  ...
  async function pauseListing(){
    if(!userId)return; setActionLoading(true)
  ...
  async function deleteListing(){
    if(!userId||!confirm('Excluir este anúncio? Esta ação é irreversível.'))return
    ...
    {showEdit&&(
      <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,.6)',zIndex:100,display:'flex',alignItems:'center',justifyContent:'center',padding:16}} onClick={e=>e.target===e.currentTarget&&setShowEdit(false)}>
        <div style={{background:'#fff',borderRadius:20,padding:24,width:'100%',maxWidth:500,maxHeight:'90vh',overflowY:'auto'}}>
          ...
          <input value={editForm.phone} onChange={e=>setEditForm(f=>({...f,phone:e.target.value}))} placeholder="21 99999-9999" inputMode="tel" style={{width:'100%',padding:'10px 13px',border:'1.5px solid #E0DDD8',borderRadius:10,fontSize:14,fontFamily:'Archivo,sans-serif',outline:'none',marginBottom:16,boxSizing:'border-box'}}/>
