...
export default function EmpregosPageClient({ initialListings }: { initialListings: Listing[] }){
  const [listings]=useState<Listing[]>(initialListings)
  const [filtered,setFiltered]=useState<Listing[]>(initialListings.filter(l=>roleOf(l)==='oferece'))
  ...
  async function reload(){
    ...
    const list=(data||[]) as Listing[];setFiltered(list.filter(l=>roleOf(l)===role))
  }
  ...
  return(<>
    ...
    <div className="hero"><div className="hi">
      {CATEGORY_IMAGES.empregos && (
        <div className="he"><Image src={CATEGORY_IMAGES.empregos} alt="" fill sizes="74px" unoptimized style={{objectFit:'cover'}} /></div>
      )}
    ...
    {showForm&&<FormModal subtypes={SUBTYPES} type="emprego" userId={userId!} initialRole={role} onClose={()=>setShowForm(false)} onSaved={()=>{setShowForm(false);reload()}}/>}
  </>)
}
