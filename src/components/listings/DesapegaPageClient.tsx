...
const SUBTYPES:[string,string][] = [['venda','Venda'],['doacao','Doação'],['troca','Troca']]
...
export default function DesapegaPageClient({ initialListings }: { initialListings: Listing[] }){
  const [listings]=useState<Listing[]>(initialListings)
  const [filtered,setFiltered]=useState<Listing[]>(initialListings)
  const [filter,setFilter]=useState('todos')
  const [userId,setUserId]=useState<string|null>(null)
  const [search,setSearch]=useState('')
  const [showForm,setShowForm]=useState(false)

  // Sessão fica no localStorage do navegador — só dá pra saber quem
  // está logado aqui no cliente, depois que a lista (pública) já veio
  // pronta do servidor.
  useEffect(()=>{supabase.auth.getSession().then(({data:{session:s}})=>{if(s)setUserId(s.user.id)})},[])

  async function reload(){
    const{data}=await supabase.from('listings').select('id,title,description,price,price_label,address,subtype,created_at,status,user:profiles(name),photos:listing_photos(url,order)').eq('type','desapega').eq('status','active').order('created_at',{ascending:false})
    const list=(data||[]) as Listing[];setFiltered(list)
  }
  ...
  return(<>
    ...
    <div className="hero"><div className="hi">
      {CATEGORY_IMAGES.desapega && (
        <div className="he"><Image src={CATEGORY_IMAGES.desapega} alt="" fill sizes="74px" unoptimized style={{objectFit:'cover'}} /></div>
      )}
      <div><div className="hn">DESAPEGA</div><div className="hc"><span>{filtered.length}</span> itens disponíveis</div></div>
    </div></div>
    ...
    <div className="page">
      <div className="sw"><div className="sw-row">
        <div className="sb">
          ...
          <input type="text" placeholder="Buscar no desapega..." value={search} onChange={handleSearch}/>
        ...
        <button className="btnp" onClick={()=>userId?setShowForm(true):window.location.href='/login'}>+ Anunciar item</button>
      </div></div>
      ...
      <div className="rc">Mostrando <span>{filtered.length}</span> itens</div>
          ...
          {filtered.map(l=>{
            ...
            return(
              <a key={l.id} className="card" href={`/anuncio/${l.id}`}>
                <div className="ci">{cover?<Image unoptimized src={cover} alt={l.title} fill sizes="(max-width:639px) 50vw, (max-width:1023px) 33vw, 300px" style={{objectFit:'cover'}} />:<span>🏷️</span>}
                  {badge&&<div style={{position:'absolute',top:8,left:8,padding:'2px 8px',borderRadius:6,fontSize:9,fontWeight:700,background:'#FEF3E2',color:'var(--sign-dark)'}}>{badge[1]}</div>}
                </div>
                <div className="cb">
      ...
      {filtered.length===0&&(
        <div className="empty">
          ...
          <div style={{fontSize:16,fontWeight:600,color:'#555',marginBottom:6}}>{search?`Nenhum resultado para "${search}"`:'Nenhum item ainda'}</div>
          <div style={{fontSize:13,color:'#AAA'}}>{search?'Tente outro termo.':'Seja o primeiro a publicar!'}</div>
          {!search&&<button className="btnp" style={{marginTop:16}} onClick={()=>userId?setShowForm(true):window.location.href='/login'}>+ Anunciar item</button>}
        </div>
      )}
    ...
    {showForm&&<FormModal subtypes={SUBTYPES} type="desapega" userId={userId!} onClose={()=>setShowForm(false)} onSaved={()=>{setShowForm(false);reload()}}/>}
  </>)
}
...
function FormModal({subtypes,type,userId,onClose,onSaved}:{subtypes:[string,string][];type:string;userId:string;onClose:()=>void;onSaved:()=>void}){
  ...
  return(
    <div className="mbg" onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div className="modal">
        <div className="mt">Novo anúncio — Desapega</div>
