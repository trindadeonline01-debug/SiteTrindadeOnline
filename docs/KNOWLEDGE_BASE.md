...
PASSO 3.1 — Baixe a foto também como arquivo (não confie só na URL)
Além de colocar a URL no campo foto_url do CSV, baixe cada foto como arquivo de verdade (pela função de download do navegador), salvando com o nome do arquivo igual ao nome do produto (ex: produto "Burguer 01" → arquivo "Burguer 01.jpg"), mantendo a extensão original. Salve tudo na pasta padrão de Downloads.
Por quê: muitos CDNs de cardápio (Anota Aí, Cardápio Web, Jotaja) aceitam o nosso servidor baixando a foto pela URL do CSV — mas alguns (ex: images.brendi.com.br) bloqueiam esse tipo de acesso com erro 403, mesmo a URL sendo pública e funcionando normal no navegador. Baixando o arquivo agora, nessa mesma rodada, elimina o vai-e-volta de descobrir a falha só depois de importar o CSV e ter que rodar um segundo prompt separado só pra isso — a foto baixada vira o plano B pronto na hora, e é só importar pelo "🖼️ Importar fotos pelo nome do produto" pra quem a URL falhar.
...
PASSO 6 — Entregar
Me devolva o CSV completo, pronto pra eu copiar/colar ou baixar como arquivo .csv — não resuma, não pule produtos, não trunque. Confirme também quantas fotos foram baixadas como arquivo (PASSO 3.1) e liste as que falharam, se houver.
```
...
### Importador universal de cardápio com IA (set/2026)
Self-service da importação acima — em vez do lojista depender do Ricardo rodando o prompt na extensão Claude for Chrome, o painel (`/painel/catalogo` → Importar ▾ → "🤖 Importar cardápio com IA") lê direto: **link** de qualquer site de cardápio (iFood, Anota Aí, Cardápio Web, Goomer, outro), **PDF** ou **fotos** de um cardápio físico. Não é um scraper por plataforma (frágil, quebra a cada mudança de layout) — é a mesma IA (Claude, modelo `claude-sonnet-5`) lendo o conteúdo e devolvendo produto/categoria/preço/complementos estruturado, com prévia de contagem ("8 categorias, 73 produtos, 41 imagens, 18 grupos encontrados") pra revisar antes de importar de verdade.
- Rota: `POST /api/painel/importar-ia` — recebe `source: 'url'|'pdf'|'fotos'`, chama a API da Anthropic com `output_config.format` (Zod) pra sair já estruturado, e converte pro **mesmo formato de CSV** que "Importar de um CSV" já consome (`nome,categoria,descricao,preco,grupos,foto_url`). A confirmação no painel gera um `File` desse CSV e chama `handleImportCsv` direto — reaproveita 100% do pipeline que já existia (undo da última importação, modo "atualizar existentes", download+rehost de foto por URL, progresso/pausa/cancelamento).
- **Depende de `ANTHROPIC_API_KEY` configurada na Vercel** (nome exato da env var) — sem isso a rota responde erro 500 explicando o que falta. Ricardo precisa gerar essa chave em console.anthropic.com e colar na Vercel (mesmo processo já feito pra outras chaves como a do OpenRouteService).
- **Link só funciona bem em site que manda conteúdo no HTML.** Fetch server-side simples não executa JavaScript — cardápio que carrega tudo via app/JS pesado (app do iFood, Anota Aí em alguns casos) pode devolver página vazia; a rota detecta isso (texto extraído curto demais) e devolve erro pedindo pra tentar PDF ou fotos em vez de importar lixo. O fluxo manual via extensão Claude for Chrome (documentado acima) continua sendo o fallback pra esses casos, porque ele navega a página de verdade.
- Custo por importação de ~80 produtos: na faixa de R$1 a R$3 (Sonnet 5), negligível perto da mensalidade do plano Loja/Operação.
...
9. ~~Sistema de entrega (delivery)~~ — **v1 implementado em ago/2026**, ver seção 6 "Trindade Entrega". Reestruturação de planos/preços puxada por isso — **detalhada no item 12 abaixo** (set/2026): mensalidade única de R$199/mês substituindo o Plano Visibilidade, mais reforma da economia de entrega (diária, escalonamento por volume, split motoboy/plataforma). Ainda com uma decisão pendente do Ricardo antes de implementar.
...
12. **Novo modelo de monetização — mensalidade única + reforma da economia de entrega** (discutido com Ricardo, set/2026 — "segura um momento e já documenta essa ação". **Nada disso foi implementado ainda** — é só o plano combinado, aguardando a decisão pendente abaixo antes de mexer em schema/cobrança. Detalha e substitui a observação solta que estava no item 9.)
    ...
    **Economia de entrega (Trindade Entrega) — pode ser feita independente da decisão acima ("Frente B")**
    1. Diária passa a ser cobrada em **qualquer** entrega confirmada no dia — hoje só cobra em pedido avulso (sem `pedido_id`); pedido vindo do cardápio/checkout da própria plataforma hoje não paga diária nenhuma, só desconta o crédito da corrida (ver `src/app/api/entrega/webhook/route.ts`, bloco `diariaConsumidaAgora`)
    2. Cobrança por volume escalonada: a diária de R$10 já cobre as 10 primeiras entregas do dia; cada entrega além da 10ª cobra R$0,50 a mais (ex: 30 entregas no dia = R$10 diária + R$10 de 20 extras = R$20 no dia; 200 entregas = R$10 + R$95 de 190 extras = R$105 no dia)
    3. Split motoboy/plataforma por entrega — **conferido no código, não existe hoje**: o motoboy recebe 100% do valor da entrega, o mesmo número que a loja paga (`src/app/api/admin/motoboy-payouts/route.ts`, `cur.valor += Number(o.fee)` — sem desconto nenhum). Nova regra: de cada corrida, R$1,00 fica retido pra plataforma, o resto vai pro motoboy (ex: entrega de R$5,00 → R$4,00 motoboy, R$1,00 plataforma)

    **Pontos de atenção do próprio Ricardo pra quando isso for implementado:** não pode quebrar nada do que já existe, nem gerar lentidão/excesso de requisições — reaproveitar padrões já existentes no código (tabela de faixa configurável tipo `entrega_bairros`/`entrega_km_tiers`, motor de cobrança Pix do Plano Visibilidade) em vez de criar mecanismos novos do zero.
...
- **Foto 403 na importação de cardápio via CSV (JBurguer/Brendi, set/2026):** 55 de 57 produtos importados sem foto, todos com o mesmo erro "URL não respondeu (status 403)" — CDN `images.brendi.com.br`. Diferente do caso do Anota Aí (foto em baixa resolução, mas baixava), aqui o `/api/loja/importar-foto` nem consegue baixar: o CDN recusa a requisição do nosso servidor inteira. Indício forte de bloqueio por reputação de IP de datacenter (não só checagem de Referer) — até o proxy do ambiente sandbox do Claude Code bloqueia esse domínio por política própria, sinal de que é um domínio tratado como suspeito por classificador de tráfego automatizado de forma ampla, não algo específico da Vercel. Nesses casos, ajustar header não resolve. Duas mudanças feitas:
  1. `/api/loja/importar-foto` agora tenta 2 variantes de header antes de desistir (com e sem `Referer`) — barato, resolve o caso de CDN que só é exigente com Referer, não ajuda quando é bloqueio por IP.
  2. O prompt padrão de extração ganhou o **PASSO 3.1 — Baixe a foto também como arquivo**: em vez de confiar só na URL indo pro CSV e descobrir a falha depois (exigindo rodar um segundo prompt separado só pra baixar foto), a mesma rodada da extensão já baixa cada foto como arquivo com o nome do produto, pronta pra importar pelo "🖼️ Importar fotos pelo nome do produto" assim que aparecer qualquer aviso de foto não importada — sem ida e volta.
