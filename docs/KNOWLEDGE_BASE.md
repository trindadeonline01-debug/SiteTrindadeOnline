...
12. **Novo modelo de monetização — mensalidade única + reforma da economia de entrega** (discutido com Ricardo, set/2026 — "segura um momento e já documenta essa ação". Detalha e substitui a observação solta que estava no item 9. **Frente A ainda não implementada** — aguardando a decisão pendente abaixo antes de mexer em schema/cobrança. **Frente B já implementada** — ver detalhes no final da subseção, aprovada por Ricardo com "Frente B ok tb".)
    ...
    **Economia de entrega (Trindade Entrega) — "Frente B", implementada em set/2026, aprovada por Ricardo ("Frente B ok tb")**
    ...
    2. ✅ Cobrança por volume escalonada: a diária já cobre as N primeiras entregas do dia (`entrega_pricing.diaria_inclui`, default 10); cada entrega além dessa cobra um extra fixo (`entrega_pricing.diaria_extra_valor`, default R$0,50) descontado do crédito em R$ da carteira (ex: 30 entregas no dia = diária + 20×R$0,50 = R$10 de extra; 200 entregas = diária + 190×R$0,50 = R$95 de extra)
    3. ✅ Split motoboy/plataforma por entrega: de cada corrida, `entrega_pricing.motoboy_corte_plataforma` (default R$1,00) fica retido pra plataforma, o resto vai pro motoboy (ex: entrega de R$5,00 → R$4,00 motoboy, R$1,00 plataforma) — aplicado na mensagem de confirmação ao motoboy, na prévia de repasses pendentes do admin e no cálculo real do repasse

    Reaproveitou a tabela `entrega_pricing` já existente (3 colunas novas) e a mecânica de crédito/diária da carteira já existente, em vez de criar tabelas novas — conforme pedido do Ricardo. Pontos de código: `src/app/api/entrega/webhook/route.ts` (cálculo e cobrança no momento da confirmação do código de entrega), `src/lib/entregaDispatch.ts` (diária agora exigida em qualquer entrega, não só avulsa, na criação da corrida), `src/app/api/admin/motoboy-payouts/route.ts` (split aplicado na prévia e na geração do repasse), `src/lib/entregaPricing.ts` (`getEntregaPricing()` com os 3 campos novos).
