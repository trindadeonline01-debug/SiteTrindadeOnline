...
const DIARIA_VALUE = 30.00
const CREDIT_PACKS: Record<number, number> = { 10: 50.00, 20: 100.00, 50: 250.00 }

// Cria a cobrança Pix da diária de entrega ou de um pacote de créditos.
// A carteira (company_delivery_wallet) só é creditada quando o Pix cai de
// verdade — ver o branch `delivery_wallet` em /api/mp/webhook.
export async function POST(req: NextRequest) {
  try {
    const { access_token, company_id, kind, credits } = await req.json()
    ...
    if (kind === 'diaria') {
      ...
      creditsToBuy = Number(credits)
      value = CREDIT_PACKS[creditsToBuy]
      if (!value) return NextResponse.json({ error: 'pacote de créditos inválido' }, { status: 400 })
    }
    ...
    const description = kind === 'diaria'
      ? `Trindade Entrega — Diária (${company.name})`
      : `Trindade Entrega — ${creditsToBuy} entregas (${company.name})`
    ...
    const res = await fetch('https://api.mercadopago.com/v1/payments', {
      ...
      headers: {
        ...
        'X-Idempotency-Key': `entrega-${company_id}-${kind}-${creditsToBuy}-${Date.now()}`,
      },
      body: JSON.stringify({
        ...
        external_reference: JSON.stringify({ type: 'delivery_wallet', company_id, kind, credits: creditsToBuy }),
      }),
    })
    ...
    await supabase.from('delivery_payments').insert({
      payment_id: String(data.id), company_id, kind, credits: creditsToBuy, value, status: 'pending',
    })
