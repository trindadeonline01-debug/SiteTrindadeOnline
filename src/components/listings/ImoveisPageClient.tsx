...
const SUBTYPES:[string,string][] = [['aluguel','Aluguel'],['venda','Venda']]
...
export default function ImoveisPageClient({ initialListings }: { initialListings: Listing[] }){
  ...
  async function reload(){
    const{data}=await supabase.from('listings').select('id,title,description,price,price_label,address,subtype,created_at,status,user:profiles(name),photos:listing_photos(url,order)').eq('type','imovel').eq('status','active').order('created_at',{ascending:false})
  ...
  return(<>
    ...
    <div className="topbar"><div className="ti">
      <div/>
      <div className="bc"><a href="/">Início</a><span className="bcs">›</span><span className="bcc">Imóveis</span></div>
      <div/>
    </div></div>

    <div className="hero"><div className="hi">
      {CATEGORY_IMAGES.imoveis && (
        <div className="he"><Image src={CATEGORY_IMAGES.imoveis} alt="" fill sizes="74px" unoptimized style={{objectFit:'cover'}} /></div>
      )}
      <div><div className="hn">IMÓVEIS</div><div className="hc"><span>{filtered.length}</span> itens disponíveis</div></div>
    </div></div>
    ...
    <div className="page">
      <div className="sw"><div className="sw-row">
        <div className="sb">
          ...
          <input type="text" placeholder="Buscar no imóveis..." value={search} onChange={handleSearch}/>
        ...
        <button className="btnp" onClick={()=>userId?setShowForm(true):window.location.href='/login'}>+ Anunciar imóvel</button>
      </div></div>
      ...
      {filtered.length===0&&(
        <div className="empty">
          ...
          {!search&&<button className="btnp" style={{marginTop:16}} onClick={()=>userId?setShowForm(true):window.location.href='/login'}>+ Anunciar imóvel</button>}
        </div>
      )}
    ...
    {showForm&&<FormModal subtypes={SUBTYPES} type="imovel" userId={userId!} onClose={()=>setShowForm(false)} onSaved={()=>{setShowForm(false);reload()}}/>}
  </>)
}
