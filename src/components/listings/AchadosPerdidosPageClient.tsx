...
const SUBTYPES:[string,string][] = [['perdido','Perdido'],['achado','Achado']]
...
export default function AchadosPerdidosPageClient({ initialListings }: { initialListings: Listing[] }){
  ...
  async function reload(){
    const{data}=await supabase.from('listings').select('id,title,description,price,price_label,address,subtype,created_at,status,user:profiles(name),photos:listing_photos(url,order)').eq('type','achado').eq('status','active').order('created_at',{ascending:false})
  ...
  return(<>
    ...
    <div className="topbar"><div className="ti">
      <div/>
      <div className="bc"><a href="/">Início</a><span className="bcs">›</span><span className="bcc">Achados & Perdidos</span></div>
      <div/>
    </div></div>
    ...
    <div className="page">
      ...
      {filtered.length===0&&(
        <div className="empty">
          ...
          {!search&&<button className="btnp" style={{marginTop:16}} onClick={()=>userId?setShowForm(true):window.location.href='/login'}>+ Publicar</button>}
        </div>
      )}
    ...
    {showForm&&<FormModal subtypes={SUBTYPES} type="achado" userId={userId!} onClose={()=>setShowForm(false)} onSaved={()=>{setShowForm(false);reload()}}/>}
  </>)
}
...
function FormModal({subtypes,type,userId,onClose,onSaved}:{subtypes:[string,string][];type:string;userId:string;onClose:()=>void;onSaved:()=>void}){
  ...
  return(
    <div className="mbg" onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div className="modal">
        <div className="mt">Novo anúncio — Achados & Perdidos</div>
