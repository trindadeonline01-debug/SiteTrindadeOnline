...
export default function PalavraPremiadaTab() {
  ...
  async function criar() {
    ...
    if (ativa && !confirm(`Já existe a rodada ativa com a palavra "${ativa.word}". Criar uma nova encerra essa automaticamente. Continuar?`)) return
    setLoading(true)
    const res = await fetch('/api/palavra-premiada', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ action: 'create', word, prize_description: prize, max_winners: maxWinners })
    })
    const data = await res.json()
    setLoading(false)
    if (data.error) return alert(data.error)
    setWord(''); setPrize(''); setMaxWinners(3)
    load()
  }
  ...
  const ativa = rounds.find(r => r.status === 'active')
  const winnersOf = (roundId: string) => winners.filter(w => w.prize_word_id === roundId).sort((a, b) => a.position - b.position)

  return (
    <div>
      <div style={s.card}>
        <div style={s.cardTitle}>🎁 Rodada Ativa</div>
        {ativa ? (
          <div>
            <div style={{ fontSize: 20, fontWeight: 700, color: '#111', marginBottom: 4 }}>"{ativa.word}"</div>
            <div style={{ fontSize: 13, color: '#666', marginBottom: 10 }}>Prêmio: {ativa.prize_description}</div>
            <div style={{ fontSize: 13, fontWeight: 600, color: '#C9951A', marginBottom: 14 }}>{ativa.winners_count}/{ativa.max_winners} ganhadores</div>
            {winnersOf(ativa.id).length > 0 && (
              <div style={{ marginBottom: 14 }}>
                {winnersOf(ativa.id).map(w => (
            ...
            <button style={s.btnGhost} onClick={() => encerrar(ativa.id)}>Encerrar rodada agora</button>
          </div>
        ) : (
          <div style={{ fontSize: 13, color: '#999' }}>Nenhuma rodada ativa no momento.</div>
        )}
      </div>
