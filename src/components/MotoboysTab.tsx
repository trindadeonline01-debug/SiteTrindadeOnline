...
interface Motoboy {
  ...
  active: boolean; created_at: string; terms: TermsInfo | null
}
...
const s: Record<string, any> = {
  ...
  row: { display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12, padding: '14px 0', borderBottom: '1px solid #F0EDE8' },
...
export default function MotoboysTab() {
  ...
  async function toggle(m: Motoboy) {
    ...
    await fetch('/api/motoboys', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'toggle', access_token: token, id: m.id, active: !m.active }) })
  }
  ...
  return (
    <div>
      ...
      <div style={s.card}>
        <div style={s.cardTitle}>Cadastrar motoboy na mão</div>
      ...
      <div style={s.card}>
        <div style={s.cardTitle}>Motoboys aprovados ({aprovados.length})</div>
        ...
        {aprovados.map(m => (
          <div key={m.id} style={s.row}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              ...
              <div>
                <div style={s.name}>{m.name} {m.status === 'pendencia' && <span style={s.badge('#FEF3E2', '#92600A')}>pendência aberta</span>}</div>
      ...
      {lightbox && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(21,18,16,.6)', zIndex: 300, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}
          ...
          <div style={{ background: '#fff', borderRadius: 16, maxWidth: 420, width: '100%', overflow: 'hidden' }}>
            ...
            <div style={{ aspectRatio: lightbox.url?.endsWith('.pdf') ? undefined : '4/3', background: '#FAFAF8', display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: 120 }}>
              {lightbox.url ? (
                lightbox.label.startsWith('Termo assinado')
                  ...
                  // eslint-disable-next-line @next/next/no-img-element
                  : <img src={lightbox.url} alt={lightbox.label} style={{ width: '100%', maxHeight: 380, objectFit: 'contain' }} />
