...
export default function NotificationPrompt() {
  ...
  return (
    <div style={{position:'fixed',bottom:80,left:16,right:16,zIndex:998,maxWidth:560,margin:'0 auto'}}>
      <div style={{background:'var(--ink)',borderRadius:14,padding:'14px 16px',border:'1px solid rgba(201,149,26,0.4)',boxShadow:'0 8px 24px rgba(0,0,0,0.3)',display:'flex',alignItems:'center',gap:12}}>
        ...
        <div style={{display:'flex',flexDirection:'column',gap:6,flexShrink:0}}>
          <button onClick={ativar} style={{padding:'6px 14px',background:'var(--sign)',color:'var(--ink)',border:'none',borderRadius:8,fontSize:12,fontWeight:700,cursor:'pointer',whiteSpace:'nowrap',fontFamily:'Archivo,sans-serif'}}>🔔 Ativar</button>
          <button onClick={dispensar} style={{padding:'5px 14px',background:'transparent',color:'#555',border:'none',borderRadius:8,fontSize:11,cursor:'pointer',whiteSpace:'nowrap',fontFamily:'Archivo,sans-serif'}}>Agora não</button>
        </div>
      </div>
    </div>
  )
